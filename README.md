...
- 新增“性能监测”页面，每秒可视化 CPU、系统内存、GPU 引擎、独占/共享显存、llama-server CPU/内存/GPU、磁盘与网络吞吐。
- 接入 llama-server `/metrics` 与 `/slots`，展示预填充/生成速度、累计 tokens、处理/排队请求、并发槽位、上下文用量与高水位、推测解码槽及运行时长。
- 图表保留最近 90 秒数据，支持悬停读取采样值和暂停/继续；监测完全在本机完成。
...
- 为兼容 llama.cpp，托管 Key 以本地文本文件保存在 `data/api-keys` 或当前用户应用数据目录；NTFS 下限制为当前 Windows 用户访问。
